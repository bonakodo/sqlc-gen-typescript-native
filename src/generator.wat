...
(func $generate (param $request i32)
  ...
  (call $reserve (local.get $used_files) (call $c_compact_file))
  (call $reserve (local.get $used_files) (call $c_compact_common_file))
  (call $reserve (local.get $used_files) (call $c_compact_sqlite_file))
  (call $reserve (local.get $used_files) (call $c_compact_pg_file))
  (call $reserve (local.get $used_files) (call $c_compact_mysql_file))
  (local.set $group (global.get $gen_groups))
  (block $groups_done (loop $groups
    ...
    (block $planned (loop $plans
      ...
      (if (i32.or (i32.ne (i32.load offset=12 (local.get $plan)) (i32.const 3))
                  (i32.load offset=48 (local.get $plan)))
        (then (i32.store offset=56 (local.get $m) (i32.const 1))))
    ...
    (call $compact_module_names (local.get $m))
    (call $module_begin (local.get $m))
    (if (i32.and (i32.eqz (global.get $opt_types_only)) (i32.load offset=56 (local.get $m))) (then
      (call $line (call $fmt2 (call $c_compact_file_line) (call $get_text (local.get $m) (i32.const 48))
        (call $quote (call $get_text (local.get $group) (i32.const 8)))))
